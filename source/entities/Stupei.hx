...
class Stupei extends Boss
{
    ...
    public function new(startX:Float, startY:Float) {
        super(startX, startY);
        name = "stupei";
        mask = new Hitbox(25, 50);
        startingHealth = 333;
        health = startingHealth;
        sprite = new Spritemap("graphics/stupei.png", 25, 50);
        sprite.add("idle", [0]);
        sprite.add("enrage", [1]);
        graphic = sprite;
    ...
    override public function die() {
        if(scene.getInstance("junpei") != null) {
            cast(scene.getInstance("junpei"), Junpei).enrage();
        }
        super.die();
    }
    ...
    override public function update() {
        ...
        var junpei = scene.getInstance("junpei");
        if(junpei != null) {
            var awayFromJunpei = new Vector2(
                Math.cos(getAngleTowardsEntity(junpei)),
                Math.sin(getAngleTowardsEntity(junpei))
            );
            awayFromJunpei.inverse();
            var isClose = distanceFrom(junpei, true) < 50;
            awayFromJunpei.normalize(CHASE_ACCEL / (isClose ? 1 : 4));
            velocity.add(awayFromJunpei);
        }
