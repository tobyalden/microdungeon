...
class ProcLevel extends Entity {
    ...
    public static inline var INITIAL_GRID_WIDTH = 320;
    public static inline var INITIAL_GRID_HEIGHT = 180;
    ...
    public function new() {
        ...
        // Create map data
        grid = new Grid(
            INITIAL_GRID_WIDTH, INITIAL_GRID_HEIGHT, TILE_SIZE, TILE_SIZE
        );
        randomize();
    ...
    private function mutate(choice:Int) {
        if(choice == -1) {
            ...
            drunkenWalk(true, true);
        }
        else if(choice == 23) {
            connectAllRooms(true);
        }
        else if(choice == 24) {
            drunkenWalk(false, true);
        }
        else if(choice == 25) {
            for(i in 0...HXP.choose(10, 20, 30)) {
                connectRandomly(true);
            }
        }
        else if(choice == 26) {
            for(i in 0...HXP.choose(10, 20, 30)) {
                connectRandomly(false);
            }
        }
        updateGraphic();
    }
    ...
    private function drunkenWalk(
        ...
        for(i in 0...numberOfSteps) {
            for(walker in walkers) {
                ...
                if(widePath) {
                    grid.setTile(walker.x - 1, walker.y, false);
                    grid.setTile(walker.x + 1, walker.y, false);
                    grid.setTile(walker.x, walker.y + 1, false);
                    grid.setTile(walker.x, walker.y - 1, false);

                    grid.setTile(walker.x - 1, walker.y + 1, false);
                    grid.setTile(walker.x + 1, walker.y - 1, false);
                    grid.setTile(walker.x + 1, walker.y + 1, false);
                    grid.setTile(walker.x - 1, walker.y - 1, false);
                }
    ...
    private function connectAllRooms(widePath:Bool = false) {
        while(countRooms() > 1) {
            connectTwoRooms(getRooms(), widePath);
        }
    }

    private function connectRandomly(widePath:Bool = false) {
        ...
        while (cDig.tileX != c2.tileX && cDig.tileY != c2.tileY) {
            grid.setTile(cDig.tileX, cDig.tileY, false);

            if(widePath) {
                grid.setTile(cDig.tileX - 1, cDig.tileY, false);
                grid.setTile(cDig.tileX + 1, cDig.tileY, false);
                grid.setTile(cDig.tileX, cDig.tileY + 1, false);
                grid.setTile(cDig.tileX, cDig.tileY - 1, false);

                grid.setTile(cDig.tileX - 1, cDig.tileY + 1, false);
                grid.setTile(cDig.tileX + 1, cDig.tileY - 1, false);
                grid.setTile(cDig.tileX + 1, cDig.tileY + 1, false);
                grid.setTile(cDig.tileX - 1, cDig.tileY - 1, false);
            }
    ...
    private function connectTwoRooms(
        rooms:Array<Array<Int>>, widePath:Bool = false
    ) {
        // Pick two random cells in different rooms
        var c1:Cell = getRandomCell();
    ...
    private function offsetChunks(
        ...
        for(i in 0...numberOfOffsets) {
            var directionX = HXP.choose(1, -1);
            var directionY = HXP.choose(1, -1);
            ...
            for(tileX in 0...chunkWidth) {
                for(tileY in 0...chunkHeight) {
                    grid.setTile(
                        chunkX + tileX + horizontalOffset * directionX,
                        chunkY + tileY + verticalOffset * directionY,
    ...
    override public function update() {
        ...
        var wideVersion = Key.check(Key.BACKSPACE);
        var choice = -100;
        ...
        if(Key.pressed(Key.DIGIT_1)) {
            //resetMapSize();
            //cast(scene, GameScene).resetCamera();
            choice = 8;
        }
